...
def main():
    ...
    # Shared queues and flag
    position_queue = queue.Queue(maxsize=1)  # Limit queue size to 1
    dt_queue = queue.Queue(maxsize=1)  # For dt
    running = threading.Event()
    running.set()

    # Start tracking and steering thread
    thread = threading.Thread(target=tracking_and_steering_thread, 
    ...
    try:
        while True:
            try:
                ...
                if detector.task == 2:
                    ...
                    position_queue.put(position)  # Pass position to thread
        ...
        running.clear()  # Stop thread
        thread.join()  # Wait for thread to finish
        cam.cam.release()
        try:
            stepper_yaw.close()
        except Exception as e:
            print(f"Error closing yaw stepper: {str(e)}")
        try:
            stepper_pitch.close()
        except Exception as e:
